...
camp <- fread("camp.csv",na.strings = c("NA"," ",NA))
patient <- fread("patient.csv",na.strings = c("NA"," ",NA))
first <- fread("first.csv",na.strings = c("NA"," ",NA))
second <- fread("second.csv",na.strings = c("NA"," ",NA))
third <- fread("third.csv",na.strings = c("NA"," ",NA))
#create train and test data based on information given
train <- camp[train,on="Health_Camp_ID"]
test <- camp[test,on="Health_Camp_ID"]
train <- patient[train,on="Patient_ID"]
test <- patient[test,on="Patient_ID"]
#prepare to create and add target variable in train file
first[,Target := 1L]
second[,Target := 1L]
third[Last_Stall_Visited_Number != 0, Target := 1L]
first <- attended_health_camp_1[,.(Health_Camp_ID,Patient_ID,Target)]
